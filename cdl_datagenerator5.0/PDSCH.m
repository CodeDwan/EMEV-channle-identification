function [mimo_downlink] = PDSCH(type,NRB,f_down,Vc,BSAtNum,UEAtNum,DelayS,raseed,SCS)
%PDSCH 此处显示有关此函数的摘要
...
simParameters.NRB = NRB;                  % Bandwidth in number of resource blocks (51RBs at 30kHz SCS for 20MHz BW)
simParameters.SubcarrierSpacing = SCS;    % 15, 30, 60, 120, 240 (kHz)
...
simParameters.NTxAnts = BSAtNum;                      % Number of PDSCH transmission antennas
if simParameters.PDSCH.NLayers > 4              % Multicodeword transmission
    ...
    simParameters.NRxAnts = UEAtNum;                      % Number of UE receive antennas
end
...
%% 计算多普勒频移
v = Vc;                    % UT velocity in km/h                
c = physconst('lightspeed'); % speed of light in m/s
fd_down = ((v*1000/3600)/c) * f_down;     % UT max Doppler frequency in Hz
...
if strcmpi(simParameters.ChannelType,'CDL')
    ...
    channel.DelayProfile = type;
    channel.DelaySpread = DelayS; % in seconds
    channel.MaximumDopplerShift = fd_down; % in Hz
    %TODO
    ...
    channel.Seed = raseed;
    channel.CarrierFrequency = f_down;
